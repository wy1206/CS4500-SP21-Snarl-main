...
class Client:
    """
    A Snarl client available for user to register and play, the default ip and port are listed below
    ...
    @staticmethod
    def is_player_update_message(req):
        """
        check whether the req is player update message
        :param req: dictionary representing the player update message
        :return: Boolean
        """
        return type(req) == dict and "type" in req and req["type"] == "player-update"
    ...
    def send_name(self):
        ...
        :return: None
        """
        my_username = input("Enter your name and the dungeon begins: ")
        username = json.dumps({"type": "player",
                               "name": my_username})
        self.send(username)
    ...
    def update(self, pos, board):
        ...
        for i in range(0, len(self.vis_board)):
            temp_row = []
            for j in range(0, len(self.vis_board[0])):
                ...
                for obj in self.vis_board[j][i].occupied_by:
                    if isinstance(obj, Player) and not (self.is_expelled and j == 2 and i == 2):
                        if obj.player_id == 0:
    ...
    def open_window(self):
        ...
        pygame.display.set_caption("Snarl Player")
            ...
            try:
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.KEYDOWN:
                            ...
                            if event.key == pygame.K_RETURN:
                                move_send = {"type": "move",
                                             "by": "player",
                                             "to": None if self.selected_pos == (2, 2)
                                             else [(self.player_pos[1] + self.selected_pos[1] - 2),
                                                   (self.player_pos[0] + self.selected_pos[0] - 2)]}
                ...
                for i in range(0, len(self.tile_gui)):
                    for j in range(0, len(self.tile_gui[0])):
                        self.screen.blit(self.tile_gui[j][i], (i * 100, j * 100))
                ...
                if self.can_move:
                    self.screen.blit(self.selected_target, (self.selected_pos[0] * 100, self.selected_pos[1] * 100))
    ...
    def server_listener(self):
        ...
        :return: None
        :except: JSONDecodeError: when the message received is not well-formed.
        """
        while self.is_running:
            data = self.receive()
            if not data:
            ...
            if self.is_level_start(req):
                ...
                for i in player_list:
                    temp.append(i)
                print(f"Game started. The dungeon has {level_num} levels. The players are {temp}")
            if self.is_player_update_message(req) and not self.initial_updated:
                pos, board, expelled_status = jf.parse_player_update_message(req)
                if expelled_status:
            ...
            if self.is_player_update_message(req) and self.initial_updated:
                if req["message"] == "Key":
