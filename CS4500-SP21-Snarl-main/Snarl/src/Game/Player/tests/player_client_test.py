...
# level4
room11 = Room((0, 0), 5, 4, [(1, 1), (2, 1), (1, 2), (2, 2), (3, 2)], [(3, 2)])
...
class PlayerClientTestCase(unittest.TestCase):
    ...
    @staticmethod
    def test_player_client1():
        """
        The test which open the GUI window for player client. Notice that the output cna be ignored.
        :return: None
        """
        state1 = GameState(5, 0, False, [], [], 7)
        gm1 = GameManager([], [], state1, None)
        gm1.preload_levels([level3])

        gm1.register_player("Bob")
        gm1.register_player("Catherine")
        gm1.register_player("Dan")
        gm1.register_player("Edward")
        gm1.set_level()
        gm1.curr_state.init_character()
        gm1.update()
